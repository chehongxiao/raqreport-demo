		...
		if (es.type == 1) {
			...
			if (es.calendarType == 1 || es.calendarType == 8) {
				doms.find('._VALUE_').datetimepicker({
					...
					changeYear: true,
					dateFormat : 'yy-mm-dd',
					timeFormat : (es.calendarType==8?'hh:mm:ss':'hh:mm'),
					showSecond: true,
					hourGrid: 4,
					minuteGrid: 10,
					secondGrid: 10
				...
				doms.find('._VALUE_').datepicker({
					changeMonth: true,
					changeYear: true,
					dateFormat : es.calendarType==2?'yy-mm-dd':(es.calendarType==4?'yy-mm':(es.calendarType==5?'yy':(es.calendarType==6?'mm':'dd')))
					,onClose:function(s,obj){
						myblur();
					}
				});
			...
			//div.remove();
			if (div.length==0) {
				div = $("<div style='position:absolute;display:none;'><div style='overflow-y:auto;overflow-x:hidden;background-color:#F8F8F8;width:auto;height:300px;' id='_es_tree_div_"+esIdx+"'></div></div>");
				$('body').append(div);
			}
