...
/*
	...
	refresh : function() {
		...
		var tbody = $('<table border=0 style="table-layout:fixed;border:0;border-collapse:collapse;border:0px;margin:'+(tableDivHasTopBut?50:25)+'px 25px 0;" cellspacing=0 cellpadding=0></table>');
...
$(document).ready(function(){
	...
	if (!guideConf.view) {
		...
		for (var i=0; i<rqAnalyse.dataSets.length; i++){
			...
			var ft = function(dsi) {
				var fileExist = false;
				if (dsi.dataId == '') dsi.dataId = guideConf.dataFolderOnServer+"temp/q"+new Date().getTime()+(guideConf.dataFileType.toLowerCase()=="binary"?".bin":".txt");
				else {
					$.ajax({
						type: 'POST',
						async : false,
...
function manageDataSet(){
	...
	var dsClick = function(){
		...
		currDataSet.find('input[dataId]').val(ds.dataId);
		...
		var saveFunc = function(silence){
			...
			ds.type = type;
			if (type != 6) ds.dataId = currDataSet.find('input[dataId='+type+']').val();
	...
	addBut.click(function(){
		...
		rqAnalyse.dataSets.push({
			...
			,tableName:''
			,dqlConf:{}
			,dataId:guideConf.dataFolderOnServer+'temp/'+new Date().getTime()+"."+(guideConf.dataFileType=='text'?'txt':'bin')//缓存的数据ID
			,rowCount:0
		});
