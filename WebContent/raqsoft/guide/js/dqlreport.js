...
*/
	,getDql : function(conf, rd) {
		...
		for (var i=0; i<conf.fields.length; i++)
		{
			...
			if (fieldi._fieldType == 'analyse') continue;
			if (fieldi._status != '') {
				alert('请先删除无效字段');
				return null;
			}
		...
		if (name == '')
		{
			...
			for (var i=aggrNames.length-1; i>=0; i--)
			{
				//if (!(f._fieldType == 'aggr' && aggrs[i] == f.aggr)){
					name = name.replaceAll(aggrNames[i],'');
				//}
			}
...
//		lefts:[{name:'',macroName:'',srcItems:[itemId0,itemId1],exp:'itemId0/itemId1',aggrExp:'sum(itemId0)/sum(itemId1)',use:1,order:0无序/1升序/2降序,groups:[lefts,tops里的分组，空分组表示整体聚合]/null表示随分组自动聚合,analyse:{analyseName:'占比/排名/比上期/比同期',field:'被分析的测度字段',scopeGroups:[空则表示针对全部]},format:'',dataType:'',_parentType:'top/left/field',_fieldType:'group/detail/aggr/analyse',_status:'为空表示正确，不为空是失效的具体信息'}]
		...
		for (var i=0; i<conf.fields.length; i++) {
			...
			if (conf.type == 1) {
				...
				if (fi.analyse != null && fi.analyse.analyseName != '') {
					fi._fieldType = 'analyse';
					fi._finalType = 1;
				} else {
					if (hasGroup) {
						fi._fieldType = 'aggr';
	...
	,refresh : function(noCalc, noRefreshDialog) {
		...
		if (conf.type == 1) { //自定义报表
			...
			for (var i=0; i<conf.fields.length; i++) {
				...
				var bc = fieldi._fieldType=='aggr'?'#DDEBF8':(fieldi._fieldType=='analyse'?'#FFECB8':'');
	...
	,confField : {
		...
		,editStyle : function() {//
			...
			var selDom4 = getSelectDom(eds, eds, field.srcEdit);
	...
	,editConfItem : function(name){
		...
		if (confField._fieldType=='aggr') {
			if (confField.aggr != 'countd' && confField.aggr != 'avg') {
		...
		var func3 = function() {
			...
			if (conf.tops.length==0&&conf.lefts.length==0) {
				...
				if (conf.isRowData) {
					...
					for (var i=0; i<fields.length; i++) {
						rpt.cells.push({row:1,col:i+1,row2:1,col2:i+1,format:'',value:fields[i].name,valueExp:'',extend:16,leftMain:'',topMain:'',tip:5,adjustSizeMode:48,textWrap:1,hAlign:biaoti.hAlign,color:biaoti.color,backColor:biaoti.backColor,dispExp:''});//'230,244,255'
					...
					for (var i=0; i<fields.length; i++) {
						rpt.cells.push({row:i+1,col:1,row2:i+1,col2:1,format:'',value:fields[i].name,valueExp:'',extend:16,leftMain:'',topMain:'',tip:5,adjustSizeMode:48,textWrap:1,hAlign:biaoti.hAlign,color:biaoti.color,backColor:biaoti.backColor,dispExp:''});
				...
				if (conf.isRowData) {
					...
					for (var i=maxTop; i>=0; i--) {
						...
						for (var j=0; j<fields.length; j++) {
							...
							counti++;
							if (showAggrTitle) {
								rpt.cells.push({row:i+1,col:currCol,row2:rpt.rowCount-1,col2:currCol,format:'',value:fields[j].name,valueExp:'',extend:16,leftMain:'',topMain:'',tip:7,adjustSizeMode:48,textWrap:1,hAlign:biaoti.hAlign,color:biaoti.color,backColor:biaoti.backColor,dispExp:''});
								rpt.rows[rpt.rows.length] = {row:i+1,type:161};
							}
							...
							var cell = {row:rpt.rowCount,col:currCol,row2:rpt.rowCount,col2:currCol,format:fields[j].format,value:'',valueExp:exp,extend:16,leftMain:leftMain,topMain:topMain,tip:3,adjustSizeMode:48,textWrap:1,hAlign:zhibiaoj.hAlign,color:zhibiaoj.color,backColor:zhibiaoj.backColor,dispExp:''};
							finalCells.push(cell);
							rpt.cells.push(cell);
						}
						if (i>0) rpt.cells.push({row:i,col:conf.lefts.length+1,row2:i,col2:fields.length==0?currCol+1:currCol,format:conf.tops[i-1].format,value:'',valueExp:aly.exps(conf,conf.tops[i-1],2),extend:19,leftMain:'',topMain:'',tip:2,adjustSizeMode:48,textWrap:1,hAlign:fenzu.hAlign,color:fenzu.color,backColor:fenzu.backColor,dispExp:mdUtils.getEditExp4Rpx(conf.tops[i-1].srcEdit)});
					}
				...
				//纵向显示数据
					...
					if (rpt.colCount>1 && conf.tops.length>0) {
						...
						if (fields.length == 1) aggrNames = ","+fields[0].name;
						var value11 = leftNames + "" + topNames + "" + aggrNames;
						rpt.cells.push({row:1,col:1,row2:conf.tops.length,col2:rpt.colCount-1,format:'',value:value11,diagonal:36,valueExp:'',extend:16,leftMain:'',topMain:'',tip:6,adjustSizeMode:48,textWrap:1,hAlign:biaoti.hAlign,color:biaoti.color,backColor:biaoti.backColor,dispExp:''});
					}
					for (var i=0; i<conf.tops.length; i++) {
					...
					for (var i=maxLeft; i>=0; i--) {
						...
						for (var j=0; j<fields.length; j++) {
							...
							counti++;
							if (showAggrTitle) {
								rpt.cells.push({row:currRow,col:i+1,row2:currRow,col2:rpt.colCount-1,format:'',value:fields[j].name,valueExp:'',extend:16,leftMain:'',topMain:'',tip:7,adjustSizeMode:48,textWrap:1,hAlign:biaoti.hAlign,color:biaoti.color,backColor:biaoti.backColor,dispExp:''});
							}
							finalFields.push(fields[j]);
							var exp = "";
							...
							var zhibiaoj = zhibiao[j%zhibiao.length];
							var cell = {row:currRow,col:rpt.colCount,row2:currRow,col2:rpt.colCount,format:fields[j].format,value:'',valueExp:exp,extend:16,leftMain:leftMain,topMain:topMain,tip:3,adjustSizeMode:48,textWrap:1,hAlign:zhibiaoj.hAlign,color:zhibiaoj.color,backColor:zhibiaoj.backColor,dispExp:''};
							finalCells.push(cell);
							rpt.cells.push(cell);
						}
						if (i>0) rpt.cells.push({row:conf.tops.length+1,col:i,row2:fields.length==0?currRow+1:currRow,col2:i,format:conf.lefts[i-1].format,value:'',valueExp:aly.exps(conf,conf.lefts[i-1],2),extend:18,leftMain:'',topMain:'',tip:1,adjustSizeMode:48,textWrap:1,hAlign:fenzu.hAlign,color:fenzu.color,backColor:fenzu.backColor,dispExp:mdUtils.getEditExp4Rpx(conf.lefts[i-1].srcEdit)});
					}
					for (var i=0; i<finalFields.length; i++) {
						if (finalFields[i]._fieldType != 'analyse') continue;
						var aggrCell = "";
